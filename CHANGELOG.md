...
- **A transient network failure no longer discards an agent's reply.** Delivery retried three times on consecutive poll ticks, so a `fetch failed` lasting more than ~16 seconds exhausted every attempt and marked the message permanently failed — on any channel. Retries now back off exponentially (5s → 160s over 7 attempts, ~5 minutes of grace), and a message owed a retry holds only the rest of its own destination stream, so an outage on one destination no longer mutes replies to another.
...
- **A pasted link titles its thread from the page, not the URL.** When the triggering message is nothing but a URL, `src/thread-titling.ts` fetches the page (5s timeout, 100KB cap, HTML only) and uses its `<title>` instead of the raw link text. Falls back to the old URL-as-title behavior on any fetch failure, timeout, non-HTML response, or missing `<title>`. Named/numeric HTML entities in the title are decoded (a raw `&mdash;` no longer leaks into the thread name), and a trailing `" — Show — Site"` suffix (common on podcast pages) is dropped, keeping just the episode title.
